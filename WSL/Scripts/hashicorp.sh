...
# Logging function
log () {
	echo "[hashicorp] - $(date '+%Y/%m/%d %H:%M:%S') - $1: ${@:2}"
}

help() {
	echo "Usage: hashicorp COMMAND"
	echo ""
	...
	echo "consul, vault and nomad. See https://releases.hashicorp.com/"
	...
	echo "Examples:"
	echo ""
	echo "    $ hashicorp get_version packer"
	echo ""
	echo "gets the currently available version numbers for packer, one"
	echo "per line"
	echo ""
	echo "    $ hashicorp install terraform 0.7.0 darwin"
	echo ""
	echo "will install the MacOSX Terraform binary"
	echo ""
	echo "    $ hashicorp install terraform 0.7.0"
	echo ""
...
install_dependencies () {
	...
	log "INFO" "Installing hashicorp dependencies"
...
# Usage: ./hashicorp.sh get_version <prog_name>
get_version () {
	binary="$1"
	echo "Available versions for ${binary}"
...
# Usage: ./hashicorp install <prog_name> <prog_version>
install () {
	binary="$1"
	version="$2"
	...
	log "INFO" "Downloading & importing Hashicorp's public signing key"
	curl -s https://keybase.io/hashicorp/pgp_keys.asc | gpg --import

	log "INFO" "Verifying downloads using gpg"
	gpg --batch --verify "${sigfile}" "${shasums}" \
		&& grep "${zipfile}" "${shasums}" | shasum -a 256 -c \
		&& log "INFO" "Downloads verified" \
